...
export async function Shop_Enter(context: any) {
    ...
    if (context.eventPayload.command == "shop_enter" && context.eventPayload.item == "id") {
        const input = context.eventPayload.value
        let text = `⌛ Вы оказались в ${input.name}`
        ...
        if (user) {
            ...
            for (let i = bonus; i < data.length; i++) {
                ...
                if (checker && data[i].type != 'unlimited') {
                    const text = `✅${data[i].name}`
                    keyboard.callbackButton({ label: text.slice(0,40), payload: { command: `shop_bought`, item: "item", value: data[i], bonus }, color: 'positive' }).row()
                } else {
                    const text = `🛒${data[i].price}💰 - ${data[i].name}`
                    keyboard.callbackButton({ label: text.slice(0,40), payload: {  command: `shop_buy`, item: "item", value: data[i], bonus  }, color: 'secondary' }).row()
                }
            }
            ...
            keyboard.callbackButton({ label: '🚫', payload: { command: 'shop_cancel' }, color: 'secondary' }).row().inline().oneTime()
