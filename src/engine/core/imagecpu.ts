...
export async function Image_Text_Add_Card(context: any, x: number, y: number, text: any) {
    ...
    const lenna = await Jimp.read(`${dir}/card_cristmas.jpg`)
    ...
    const res = await lenna.resize(1687, 1077).print(font_big, x, y, ('0000000000000000'+text.id).slice(-16).replace(/\d{4}(?=.)/g, '$& ').replace(/ /g, `${' '.repeat(7)}`))
